...
def get_data(params,args):
    ...
    gg = glob.glob(f'{args.prefix}*')  # assume that there are no files starting with prefix, only directories
    ntotal = len(gg)
    assert (ntotal >= (args.ntrain+args.nvalid+args.ntest)),'Number of examples not sufficient'
    
    for ii, dirname in enumerate(gg):
        if ( ii < args.ntrain ):
            pass
        elif ( args.ntrain <= ii < (args.ntrain + args.nvalid)
...
if __name__ =='__main__':
    ...
    train_data,valid_data,test_data = get_data(params=params,args=args)
    cnn    = define_cnn()
    goodbye()
